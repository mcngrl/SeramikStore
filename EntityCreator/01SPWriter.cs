...
public static class SpWriter
{
    ...
    {
        var sb = new StringBuilder();
        EntityCreator.Helper.AppendAutoGeneratedHeaderForSQL(sb, nameof(SpWriter));
    ...
    static string GenerateUpdate(string schema, string table, List<DbColumn> cols)
    {
        ...
        var updatableColForParametre = cols
            .Where(c => !c.IsIdentity && !c.IsInsertDate() && !c.IsIsActive() && !c.IsUpdateDate())
            .ToList();

        var updatableColForQuery = cols
        ...
        sb.AppendLine(string.Join(",\n",
            updatableColForParametre.Select(c =>
                $"    @{c.Name} {DbSchemaReader.GetSqlType(c)}" +
                (c.IsNullable ? " = NULL" : "")
            )));
    ...
    static string GenerateGetById(string schema, string table, List<DbColumn> cols)
    {
        var id = cols.First(c => c.IsIdentity);
        var hasIsActive = cols.Any(c => c.IsIsActive());
        var columnsForList = cols
        .Where(c => !c.IsInsertDate() && !c.IsIsActive() && !c.IsUpdateDate())
        .ToList();
