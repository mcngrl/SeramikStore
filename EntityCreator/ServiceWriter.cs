...
public static class ServiceWriter
{
    ...
    static string GenerateCreate(string entity, List<DbColumn> cols)
    {
        ...
        foreach (var c in insertCols)
            sb.AppendLine(
                $"    cmd.Parameters.AddWithValue(\"@{c.Name}\", (object?)dto.{c.Name} ?? DBNull.Value);");
...
{{
    ...
    {
        var clr = MapClr(c);
        var nullable = clr != "string";

        if (nullable)
            return $"rdr[\"{c.Name}\"] == DBNull.Value ? ({clr}?)null : ({clr})rdr[\"{c.Name}\"]";

        return $"rdr[\"{c.Name}\"] == DBNull.Value ? null : ({clr})rdr[\"{c.Name}\"]";
    }
