...
-- Generated: 3/26/2026 11:26:39 PM
-- ===============================================
...
INSERT INTO [dbo].[ReturnDetail] ([Id], [ReturnId], [OrderDetailId], [ProductId], [UnitPrice], [ReturnQuantity])
VALUES
(38, 24, 1047, 3019, 960.00, 1),
(40, 26, 1047, 3019, 906.00, 3),
(41, 27, 1046, 3021, 15200.00, 1),
(42, 27, 1047, 3019, 960.00, 1),
(43, 27, 1048, 3008, 10.00, 1),
(44, 27, 1049, 3018, 5211.00, 9),
(45, 28, 1046, 3021, 15200.00, 1),
(46, 29, 1055, 3027, 95000.00, 1),
(47, 30, 1050, 3019, 960.00, 1),
(48, 30, 1051, 3021, 15200.00, 1),
(49, 30, 1052, 3027, 95000.00, 1),
(50, 30, 1053, 3006, 8714.00, 1),
(51, 30, 1054, 3008, 10.00, 1),
(52, 31, 1050, 3019, 960.00, 1),
(53, 31, 1051, 3021, 15200.00, 1),
(54, 32, 1051, 3021, 15200.00, 1),
(55, 33, 1057, 3016, 78545.00, 1),
(56, 33, 1058, 3020, 700.00, 1),
(57, 33, 1059, 3018, 5211.00, 1)
GO
...
INSERT INTO [dbo].[ReturnHeader] ([Id], [UserId], [ReturnRequestDate], [OrderId], [Reason])
VALUES
(24, 2004, '2026-03-15T22:28:24', 1023, N'Bekledigim ürüen degil'),
(26, 2004, '2026-03-15T23:00:00', 1023, N'Bunu da iade edeceğiöm'),
(27, 2004, '2026-03-25T20:28:59', 1023, NULL),
(28, 2004, '2026-03-25T20:39:04', 1023, N'saasasassasa'),
(29, 7009, '2026-03-26T16:04:35', 1025, N'beğenmedim'),
(30, 2004, '2026-03-26T16:47:54', 1024, NULL),
(31, 2004, '2026-03-26T16:50:37', 1024, NULL),
(32, 2004, '2026-03-26T17:00:44', 1024, NULL),
(33, 2004, '2026-03-26T22:16:15', 1026, NULL)
GO
...
BEGIN
   SELECT
		...
		ISNULL(SUM(t3.ReturnQuantity*t3.UnitPrice),0) LineReturnPriceTotal,
...
BEGIN
    ...
    INSERT INTO ReturnHeader (UserId, ReturnRequestDate, OrderId, Reason)
    VALUES (@UserId, GETDATE(), @OrderId, @Reason);
    ...
    -- 6️⃣ SUCCESS
    SELECT 1 AS Result, N'İade talebi oluşturuldu.' AS Message;
...
BEGIN
    ...
    SELECT 
        ...
        rd.OrderDetailId,
        rd.ProductId,
        rd.UnitPrice AS ReturnUnitPrice,
        rd.ReturnQuantity,
...
-- dbo.sp_ReturnRequest_Create

IF OBJECT_ID('dbo.sp_ReturnRequest_Create', 'P') IS NOT NULL
    DROP PROCEDURE [dbo].[sp_ReturnRequest_Create]
GO
-- =============================================
-- SP: sp_ReturnRequest_Create
-- Amaç: Müşteri iade talebi oluşturur
-- Tablolar: ReturnHeader, ReturnDetail
-- =============================================
CREATE  PROCEDURE [dbo].[sp_ReturnRequest_Create]
    @OrderId    INT,
    @UserId     INT,
    @Reason     NVARCHAR(500),
    @Items      NVARCHAR(MAX)   
    -- JSON formatı:
    -- [
    --   { "OrderDetailId": 1, "ProductId": 10, "ProductCode": "SEA001",
    --     "ProductName": "Beyaz Fayans", "ProductDesc": "60x60", 
    --     "UnitPrice": 45.00, "ReturnQuantity": 2, "LineTotal": 90.00 }
    -- ]


--	'[
--  { "OrderDetailId": 1046, "ProductId": 3021, "ProductCode": "URN01024",
--    "ProductName": "Gelinlik Kahvesi", "ProductDesc": "<ul><li><strong>Kahve köpüğünde saklı küçük mutluluklar,</strong></li><li><strong>Bir yudumda gülüş, bir yudumda anılar.</strong></li><li><strong>Sevgililer Günü’nde değil yalnız bugün,</strong></li><li><strong>Her sabah hatırlatır: “Bu evde sevgi hüküm sürer.”</strong></li><li><strong>İki fincan, tek kalp, paylaşılan bir ömür,</strong></li><li><strong>Sıcak bir kahve gibi… Aşk hep taze, hep dür.</strong></li></ul>", 
--    "UnitPrice": 15200.00, "ReturnQuantity": -2 },
--  { "OrderDetailId": 1047, "ProductId": 3019, "ProductCode": "A02",
--    "ProductName": "ŞURAMUTFAK 12 Parça Fincan Takımı | 6 Kişilik Finc", "ProductDesc": "<p>Detayları buraya </p><p>yazaıyorum</p><p><strong>Bold</strong></p><p><br></p><p><br></p><p><br></p>", 
--    "UnitPrice": 960.00, "ReturnQuantity": 1 }
--]'
AS
BEGIN
    SET NOCOUNT ON;
		...
		IF ISJSON(@Items) = 0
		BEGIN
			SELECT -1 AS Result, N'Detay bilgileri hatalı' AS Message;
			RETURN;
		RETURN
		END
        ...
        FROM [OrderStatus] t1
        LEFT JOIN [Status] t2 ON t1.StatusCode = t2.Code
		LEFT JOIN [OrderHeader] t3 ON t1.OrderId = t3.Id
        WHERE t1.OrderId = @OrderId
		...
		IF ISNULL(@LastStatuCode,0) <> 70
		BEGIN
			SELECT -1 AS Result, N'Sipariş bulunamadı veya iade için uygun değil.' AS Message;
			RETURN;
		END


    -- ── 3. GELEN İTEMLARI PARSE ET VE DOĞRULA ──
    -- ReturnQuantity > 0 olan item var mı?
		IF NOT EXISTS (
			SELECT 1 FROM OPENJSON(@Items)
			WITH (ReturnQuantity INT '$.ReturnQuantity')
			WHERE ReturnQuantity > 0
		)
		BEGIN
			SELECT -3 AS Result, N'Lütfen en az bir ürün için iade adedi giriniz.' AS Message;
			RETURN;
		END



    -- ReturnQuantity, OrderDetail.Quantity'yi aşıyor mu?
		IF EXISTS (
		SELECT od.Id ,od.Quantity,r.ReturnedQty AS IadeToplami,od.Quantity - ISNULL(r.ReturnedQty,0) AS KalanIade,parsed.ReturnQuantity
			FROM OPENJSON(@Items)
			WITH (
				OrderDetailId INT '$.OrderDetailId',
				ReturnQuantity INT '$.ReturnQuantity'
			) parsed
			LEFT JOIN OrderDetail od ON od.Id = parsed.OrderDetailId
			LEFT JOIN (
				SELECT OrderDetailId, SUM(ReturnQuantity) AS ReturnedQty
				FROM ReturnDetail
				GROUP BY OrderDetailId
			) r ON r.OrderDetailId = od.Id
			WHERE parsed.ReturnQuantity >
				  (od.Quantity - ISNULL(r.ReturnedQty,0))
		)
		BEGIN
			SELECT -5 AS Result, 
			N'İade adedi, kalan iade edilebilir miktardan fazla.' AS Message
			RETURN
		END
    ...
    -- Detail (sadece ReturnQuantity > 0 olanlar)
    INSERT INTO ReturnDetail (ReturnId, OrderDetailId, ProductId, ProductCode, ProductName, ProductDesc, UnitPrice, ReturnQuantity, LineTotal)
    SELECT
        @NewReturnId,
        parsed.OrderDetailId,
        parsed.ProductId,
        parsed.ProductCode,
        parsed.ProductName,
        parsed.ProductDesc,
        ...
        parsed.UnitPrice * parsed.ReturnQuantity as LineTotal
    FROM OPENJSON(@Items)
    WITH (
        OrderDetailId   INT             '$.OrderDetailId',
        ...
        ProductDesc     NVARCHAR(MAX)   '$.ProductDesc',
        UnitPrice       DECIMAL(9,2)    '$.UnitPrice',
        ReturnQuantity  INT             '$.ReturnQuantity'
    ) AS parsed
    WHERE parsed.ReturnQuantity > 0;

    SELECT 1 AS Result, N'İade talebiniz başarıyla oluşturuldu.' AS Message;
END
...
-- dbo.sp_ReturnRequest_GetByOrderId

IF OBJECT_ID('dbo.sp_ReturnRequest_GetByOrderId', 'P') IS NOT NULL
    DROP PROCEDURE [dbo].[sp_ReturnRequest_GetByOrderId]
GO

-- =============================================
-- SP: sp_ReturnRequest_GetByOrderId
-- Amaç: Sipariş detay ekranında mevcut iade talebini gösterir
-- =============================================
CREATE PROCEDURE sp_ReturnRequest_GetByOrderId
    @OrderId INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    INNER JOIN ReturnHeader rh ON rh.Id = rd.ReturnId
    WHERE rh.OrderId = @OrderId;
END
