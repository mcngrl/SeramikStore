...
-- Generated: 2/22/2026 10:06:18 PM
-- ===============================================
...
AS
BEGIN
DECLARE @LASTSTATUSCODE NVARCHAR(10)

SET NOCOUNT ON;
	SELECT TOP 1 @LASTSTATUSCODE =StatusCode
FROM OrderStatus t1
LEFT JOIN Status t2 ON t1.StatusCode = t2.Code
WHERE t1.OrderId =@OrderId
ORDER BY t1.IslemTarihi DESC

IF @LASTSTATUSCODE = '010'
BEGIN
	SELECT Code,Aciklama  FROM [dbo].[Status] WHERE Code IN ('020','070') Order By Code
END
IF @LASTSTATUSCODE = '020'
BEGIN
	SELECT Code,Aciklama  FROM [dbo].[Status] WHERE Code IN ('030','070') Order By Code
END
IF @LASTSTATUSCODE = '030'
BEGIN
	SELECT Code,Aciklama  FROM [dbo].[Status] WHERE Code IN ('040','070') Order By Code
END
IF @LASTSTATUSCODE = '040'
BEGIN
	SELECT Code,Aciklama  FROM [dbo].[Status] WHERE Code IN ('050','070') Order By Code
END
IF @LASTSTATUSCODE = '050'
BEGIN
	SELECT Code,Aciklama  FROM [dbo].[Status] WHERE Code IN ('070') Order By Code
END
